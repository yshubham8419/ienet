...
def predict(data_json, model_dir,
            ...
            PAS_ATT=False, QUS_ATT=False, config=None, tok=None):
    eval_examples = read_squad_examples(input_data=data_json, is_training=False)

    predict_feature_file = "predict.tf_record.electra.reverse"
    predict_feature_left_file = "predict_left.electra.reverse"
    if tok:
        tokenizer = tok
    else:
    ...
    def get_result(checkpoint):
        ...
        accumulate_predictions_v2(
            ...
            all_results, MAX_ANSWER_LENGTH, START_N, END_N, reverse=True)
