...
def create_v2_model(albert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("question_attention", reuse=tf.AUTO_REUSE):
        if qus_att_full:
            ...
            qus_att = tf.matmul(qus_Q, tf.transpose(qus_K, [0, 2, 1])) * question_mask

            qus_att = qus_att / tf.sqrt(tf.cast(output_shape[2], output.dtype))
            qus_att = qus_att - ((1 - question_mask) * 1e30)
            qus_att = tf.nn.softmax(qus_att)
            question_context = tf.matmul(qus_att, qus_V)
            question_context = question_context + output
            output = contrib_layers.layer_norm(question_context, begin_norm_axis=-1)
        else:
            # B X T X H
            output_att = tf.layers.dense(
                output,
                output_shape[2],
                kernel_initializer=electra_modeling.create_initializer(
                    albert_config.initializer_range),
            ...
            # B X 1 X T
            question_mask = tf.expand_dims(1 - p_mask, axis=1)
            scaled_output = output / tf.sqrt(tf.cast(albert_config.hidden_size, output.dtype))
            # B X T X T
            att = tf.matmul(output_att, tf.transpose(scaled_output, [0, 2, 1]))
            att = att * question_mask - ((1.0 - question_mask) * 1e30)
            att = tf.nn.softmax(att)
            question_context = tf.matmul(att, output)
            question_context = question_context + output
            output = contrib_layers.layer_norm(question_context, begin_norm_axis=-1)
    ...
    with tf.variable_scope("end_logits"):
        ...
        if entity_mask_full_version == 0:
        ...
        elif entity_mask_full_version == 1:
            output_alpha = tf.layers.dense(
                output,
                output.get_shape().as_list()[2],
                kernel_initializer=electra_modeling.create_initializer(
            ...
            att = tf.expand_dims(tf.matmul(output_alpha, tf.transpose(output_K, [0, 2, 1])), axis=1)
            att = att * en_msk
            att = att / tf.sqrt(tf.cast(albert_config.hidden_size, att.dtype))
            att = att - ((1.0 - en_msk) * 1e30)
            att = tf.nn.softmax(att)
            end_logits = tf.einsum("abcd,ade->abce", att, output_V)
        elif entity_mask_full_version == 2:
            end_logits = []
            with tf.variable_scope("entity_mask_%d" % 0, reuse=tf.AUTO_REUSE):
                ...
                att = att * en_msk
                att = att / tf.sqrt(tf.cast(output_shape[2], att.dtype))
                att = att - ((1.0 - en_msk) * 1e30)
                # B X K X T1 X T2
                att = tf.nn.softmax(att)
        ...
        elif entity_mask_full_version == 3:
            end_logits = []
            with tf.variable_scope("entity_mask_%d" % 0, reuse=tf.AUTO_REUSE):
...
def v2_model_fn_builder(albert_config, use_one_hot_embeddings, start_n_top,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        outputs = create_v2_model(
            ...
            end_n_top=end_n_top,
            qus_att_full=qus_att_full,
            entity_mask_full_version=entity_mask_version,
            GLU=GLU,
...
def predict(data_json, model_dir, qus_att_full=False,
            entity_mask_version=0, GLU=False, CLS_WITH_START_INFO=True,
    ...
    def get_result(checkpoint):
        ...
        accumulate_predictions_v2(
            ...
            all_results, MAX_ANSWER_LENGTH, START_N, END_N)
